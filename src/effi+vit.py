...
model1 = EfficientNet(num_classes=len(train_dataset_classes))
model2 = Vit(num_classes=len(train_dataset_classes))




model = MergedModel(model1,model2,num_classes=len(train_dataset_classes))

model.to(device)
print(id2label)

# Initialize optimizer and loss criterion
optimizer = optim.AdamW(model.parameters(), lr=config['training']['lr'],betas =(0.9, 0.999),eps =1e-8,weight_decay=0.001)
