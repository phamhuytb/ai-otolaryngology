...
# Implement K-Fold Cross-Validation on Training Data
k_folds = config['training']['k_folds']  # Number of folds for cross-validation
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_loader.dataset)):
    ...
    fold_train_loader = torch.utils.data.DataLoader(train_loader.dataset, batch_size=config['training']['batch_size_train'],
                                                    sampler=train_sampler)

    for epoch in range(total_epochs):
        train_loss, train_accuracy, train_f1_score, train_precision, train_sensitive, train_specificity, train_duration = train_one_epoch(
            model, fold_train_loader, optimizer, criterion, device)

        val_loss, val_accuracy, val_f1_score, val_precision, val_sensitive, val_specificity = validate_one_epoch(
        ...
        if val_f1_score > best_f1_score:
            ...
            torch.save(model.state_dict(), str(best_model_path) + f'/fold_{fold + 1}_weights.h5')
